...
*/
select month(ngay_ket_thuc) as thang, year(ngay_ket_thuc) as nam, count(ma_hop_dong) as so_luong_khach
from hop_dong
group by month(ngay_ket_thuc), year(ngay_ket_thuc)
order by year(ngay_ket_thuc), month(ngay_ket_thuc);
...
where (hop_dong.ngay_lam_hop_dong between '2020-10-01' and '2020-12-31')
		and dich_vu.ma_dich_vu not in (select ma_dich_vu
									from hop_dong
...
*/
select ma_dich_vu_di_kem, sum(so_luong) as so_lan_su_dung
...
select dich_vu_di_kem.ma_dich_vu_di_kem, 
        ...
        hop_dong.ma_hop_dong
from dich_vu_di_kem
join hop_dong on hop_dong.ma_dich_vu_di_kem = dich_vu_di_kem.ma_dich_vu_di_kem
